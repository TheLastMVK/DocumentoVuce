...
--  v_blob_len := DBMS_LOB.getlength(p_blob);
  ...
  v_nombre:=  TRIM(v_nombre);
   ...
   CASE (p_tipo)
   WHEN 1 THEN v_nombre:=v_nombre||'.xml';
   WHEN 2 THEN v_nombre:=v_nombre||'.xml';
   WHEN 3 THEN v_nombre:=v_nombre||'.pdf';
   WHEN 4 THEN v_nombre:=v_nombre||'.zip';
   else v_nombre:=v_nombre||'.pdf';
   END CASE;
