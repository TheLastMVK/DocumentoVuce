CREATE OR REPLACE PROCEDURE VCOBJ.Cargar_buzon_CAB (
      p_fecha       IN string
   )
   as
...
EXECUTE IMMEDIATE ('CREATE TABLE VCOBJ.A_'||p_fecha||'_TMP as 
  ...
  case WHEN a."envio_tipo" in ( 2,3) THEN null else a."emisor_id" end as emisor_id,
   case WHEN a."envio_tipo" in ( 1,3) THEN null else a."dest_id" end as dest_id from VCOBJ.A_'||p_fecha||' a,VCOBJ.adjunto ad,VCOBJ.A_'||p_fecha||'_ADJ b
    where a."codMensaje" = ad.mensaje_id and ad.adjunto_id =  b.adjunto_id (+) and b.mensaje_id is null');
   
EXECUTE IMMEDIATE ('INSERT INTO VCOBJ.A_'||p_fecha||'_ADJ (mensaje_id,adjunto_id,adjunto_tipo,nombre_archivo,uuid) 
