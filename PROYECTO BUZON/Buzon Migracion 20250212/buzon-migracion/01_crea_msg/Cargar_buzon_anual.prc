...
EXECUTE IMMEDIATE ('CREATE TABLE VCOBJ.A_'||p_fecha||' as 
...
EXECUTE IMMEDIATE ('CREATE TABLE VCOBJ.A_'||p_fecha||'_TMP as 
  ...
  case WHEN a."envio_tipo" in ( 2,3) THEN null else a."emisor_id" end as emisor_id,
   case WHEN a."envio_tipo" in ( 1,3) THEN null else a."dest_id" end as dest_id from VCOBJ.A_'||p_fecha||' a');
   
EXECUTE IMMEDIATE ('CREATE TABLE VCOBJ.A_'||p_fecha||'_ADJ as 
  SELECT  a.mensaje_id,b.adjunto_id,b.adjunto_tipo,nvl(b.nombre_archivo,''sinnombre'') as nombre_archivo,a.uuid from VCOBJ.A_'||p_fecha||'_TMP a,VCOBJ.adjunto b
  WHERE a.mensaje_id = b.mensaje_id and b.estado=''A''');

EXECUTE IMMEDIATE ('CREATE TABLE VCOBJ.A_'||p_fecha||'_USER as 
...
u.PRINCIPAL principal, u.EMPRESA_ID empresa_id, u.ENTIDAD_ID entidad_id from VCOBJ.A_'||p_fecha||'_TMP b,VCOBJ.usuario u
