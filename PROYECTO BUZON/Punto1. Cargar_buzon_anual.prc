CREATE OR REPLACE PROCEDURE VCOBJ.Cargar_buzon_anual (
      p_fecha       IN string,
      cant_ok       OUT number,
      cant_error    OUT number 
   )
   as
...
EXECUTE IMMEDIATE ('CREATE TABLE VCOBJ.A_'||p_fecha||' TABLESPACE VUCE  as 
    SELECT m.mensaje_id "codMensaje",m.envio_tipo "envio_tipo",
       ...
       CASE
          ...
          WHEN m.envio_tipo = 2 AND NVL(m.de, 1)=2287  then 8            
       ...
       nvl(m.mensaje_texto_id,0) "codPlantillaContenido",
 ...
 WHERE m.buzon_id = b.buzon_id (+)
   ...
   AND m.fecha_registro >= TO_DATE(''01/01/'||p_fecha||''',''DD/MM/YYYY'')
   AND m.fecha_registro <  ADD_MONTHS(TO_DATE(''01/01/'||p_fecha||''',''DD/MM/YYYY''), 12)
...
EXECUTE IMMEDIATE ('CREATE TABLE VCOBJ.A_'||p_fecha||'_ADJ TABLESPACE VUCE as 
  SELECT  a.mensaje_id,b.adjunto_id,b.adjunto_tipo,nvl(b.nombre_archivo,''sinnombre'') as nombre_archivo,a.uuid from VCOBJ.A_'||p_fecha||' a,VCOBJ.adjunto b
  WHERE a.mensaje_id = b.mensaje_id and b.estado=''A''');

EXECUTE IMMEDIATE ('CREATE TABLE VCOBJ.A_'||p_fecha||'_USER TABLESPACE VUCE as 
...
u.PRINCIPAL principal, u.EMPRESA_ID empresa_id, u.ENTIDAD_ID entidad_id from VCOBJ.A_'||p_fecha||' b,VCOBJ.usuario u
    WHERE (b."codUsuarioEnvio" = u.usuario_id or b."usu_mod_aud" = u.usuario_id or 
  (case WHEN a."envio_tipo" in ( 2,3) THEN null else a."emisor_id" end   = u.usuario_id)  or 
   (case WHEN a."envio_tipo" in ( 1,3) THEN null else a."dest_id" end  = u.usuario_id  )) ');
  
   
EXECUTE IMMEDIATE ('INSERT INTO VCOBJ.LOGERROR (ID,MESSAGEID,PROVIDER,CREATEDATE,CREATEUSERID,MESSAGEEVENTO,ID_HARDWARE,APP_NAME,CAMPO_01,VALOR_01)
  SELECT VCOBJ.SEQ_LOGERROR.Nextval,999999,''ORACLE'',SYSDATE,SYS_CONTEXT(''USERENV'', ''SESSION_USER''),''CARGA :'||p_fecha||''',''127.0.0.0'',''PASO_01'',''mensaje_id'',mm.mensaje_id 
  FROM (SELECT m.mensaje_id
  FROM vcobj.mensaje m,
       vcobj.buzon b,
       vcobj.mensaje_texto mt,       
       vcobj.notificacion n,
  ...
  and nvl(b.componente,nvl(mt.componente,nvl(t.componente,nvl(t1.componente,nvl(t2.componente,nvl(t3.componente,1)))))) in (1,2)
   AND m.fecha_registro <  ADD_MONTHS(TO_DATE(''01/01/'||p_fecha||''',''DD/MM/YYYY''), 12)
   MINUS 
  SELECT  a."codMensaje" mensaje_id from VCOBJ.A_'||p_fecha||' a ) mm');  
  
  cant_error := SQL%ROWCOUNT;
   
  sql_query := 'SELECT COUNT(1) FROM VCOBJ.A_'||p_fecha;

  EXECUTE IMMEDIATE sql_query INTO cant_ok;
   

  PKG_LOGERROR.SP_LOGERROR_INS(S_ID, 'ORACLE', 1,sysdate, SYS_CONTEXT('USERENV', 'SESSION_USER'), 'CARGA :'||p_fecha, sys_context('USERENV', 'IP_ADDRESS'), 'PASO_01','Cant Cargados :',cant_ok,'Cant Error :',cant_error,null,null);

    
      
END Cargar_buzon_ANUAL;
/
