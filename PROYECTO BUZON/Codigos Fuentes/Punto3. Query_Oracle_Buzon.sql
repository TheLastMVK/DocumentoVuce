SELECT m.mensaje_id "codMensaje",m.envio_tipo "envio_tipo",
       ...
       m.fecha_registro "fechahoraEnvio",
       nvl(m.usuid_reg_aud,441740) "codUsuarioEnvio",
       ...
       NVL(d.fecha_lectura,m.fecha_registro) "fechaLeido",
        case WHEN m.envio_tipo in ( 2,3) then e.entidad_id
          ...
          else nvl(m.de,441740) end "emisor_id",
        ...
        case WHEN m.envio_tipo in ( 1,3) then e.entidad_id
             WHEN m.envio_tipo = 2 or m.notificacion_id is not null then nvl(m.de,441740)
             else nvl(d.destinatario,441740) END "dest_id", 
 ...
 WHERE m.buzon_id = b.buzon_id (+)
   ...
   AND CASE
    ...
    AND m.fecha_registro >= TO_DATE('01/01/2022','DD/MM/YYYY')
   AND m.fecha_registro < = TO_DATE('02/01/2022','DD/MM/YYYY')  
   AND m.usuid_reg_aud = 457411 
